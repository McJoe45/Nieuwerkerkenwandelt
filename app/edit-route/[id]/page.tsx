...
import { getRouteById, updateRoute, isAuthenticated } from "@/lib/auth"
...
export default function EditRoutePage() {
  ...
  useEffect(() => {
    ...
    setIsLoggedIn(true)
    const route = getRouteById(params.id as string)

    if (route) {
      setFormData({
        id: route.id,
        name: route.name,
        gehuchten: route.gehuchten.length > 0 ? route.gehuchten : [""],
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <Link href={`/route/${formData.id}`} className="flex-1">
                  <Button
                    type="button"
                    variant="outline"
